...
- **Never set `uniqueChoices` / `uniqueValues` on a multichoice question.** The named `multichoice` type derives the **dense** layout (one 0/1 field per choice), and the scrutinizer applies uniqueness to raw field values — so a dense ballot over more than two choices always repeats a value and is discarded at tally. The election then accepts votes and reports an all-zero result while `voteCount` keeps rising. `client.elections.create/update` forces `typeSetup.uniqueChoices` to `false` and rejects an explicitly unsatisfiable `ballotProtocol`; `encodeQuestionBallot` refuses to encode for such a question. Detect an already-created broken one with `unsatisfiableQuestionReason(question)` from `@vocdoni/ballot`.
